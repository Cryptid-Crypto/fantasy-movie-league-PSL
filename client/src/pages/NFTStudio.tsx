...
import {
  ...
  Plus, Send, Coins, Package, Trophy, Tag
} from "lucide-react";
...
export default function NFTStudio() {
  ...
  const { data: treasuryHoldings } = trpc.nftPlatform.getTreasuryHoldings.useQuery();
  ...
  const listFromTreasuryMutation = trpc.nftPlatform.listFromTreasury.useMutation({
    onSuccess: () => {
      toast.success("Treasury card listed on the marketplace!");
      utils.nftPlatform.getTreasuryHoldings.invalidate();
      utils.nftPlatform.getTreasuryAccount.invalidate();
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <Tabs defaultValue="mint">
            ...
            <TabsContent value="treasury" className="space-y-6 mt-6">
              ...
              {/* TREASURY HOLDINGS — cards owned by the Treasury account, listable for sale */}
              {treasuryHoldings && treasuryHoldings.length > 0 && (
                <div className="space-y-3 pt-4 border-t border-border">
                  <p className="text-sm text-muted-foreground">
                    {treasuryHoldings.length} card{treasuryHoldings.length === 1 ? "" : "s"} held by the Treasury account — list them on the marketplace
                  </p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {treasuryHoldings.map((card: any) => (
                      <Card key={card.id} className="overflow-hidden group">
                        <div className="aspect-[3/4] bg-muted relative overflow-hidden">
                          {card.cardImageUrl ? (
                            <img src={card.cardImageUrl} alt={card.performerName} className="w-full h-full object-cover" />
                          ...
                          <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <Button
                              ...
                              onClick={() => { setListDialog({ open: true, card }); setListPrice(""); }}
                            >
      ...
      {/* LIST FROM TREASURY DIALOG */}
      <Dialog open={listDialog.open} onOpenChange={(o) => setListDialog({ open: o })}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>List Treasury Card for Sale</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            {listDialog.card && (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Listed as the Treasury account (user #{treasuryAccount?.user?.id ?? "—"}). Sale proceeds go to the Treasury balance.
              </p>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              disabled={!listPrice || parseInt(listPrice) < 1 || listFromTreasuryMutation.isPending}
              onClick={() => {
                if (!listDialog.card || !listPrice) return;
                listFromTreasuryMutation.mutate({ cardId: listDialog.card.id, priceCredits: parseInt(listPrice) });
              }}
            >
              {listFromTreasuryMutation.isPending ? <><RefreshCw className="h-4 w-4 animate-spin mr-2" />Listing...</> : <><Tag className="h-4 w-4 mr-2" />List for Sale</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
