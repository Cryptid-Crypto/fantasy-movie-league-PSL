/**
 * Integration tests for Tournament Roster Entry using the real DB layer.
 * These are kept in a separate file from tournament.entry.test.ts because
 * that file uses vi.mock('./db') for unit tests, which would intercept the
 * real DB calls needed here.
 */
...
describe('Tournament Roster Entry', () => {
  ...
  beforeAll(async () => {
    // Create test user
    await db.upsertUser({
      openId: 'test-user-roster-entry-integration',
      name: 'Test User Integration',
      email: 'test-integration@example.com',
    });
    const testUser = await db.getUserByOpenId('test-user-roster-entry-integration');
    testUserId = testUser!.id;

    // Create test performers
    testPerformer1Id = await db.createPerformer({
      name: 'Test Performer 1 Integration',
      performerType: 'Legend',
    });
    testPerformer2Id = await db.createPerformer({
      name: 'Test Performer 2 Integration',
      performerType: 'Anal Queen',
    });

    // Create test tournament
    testTournamentId = await db.createTournament({
      name: 'Test Roster Tournament Integration',
  ...
  it('should include performer types in entry performers query', async () => {
    ...
    expect(legendPerformer!.performerName).toBe('Test Performer 1 Integration');
  });
});
